...
mzextractor <- function(files, imzMLparse, thres.int= 10000,
  ...
  summary <- paste(length(f.s.mz),"unique ions retained within between",
                   ...
                   "unique ions detected across all pixels.", sep=" ")
...
subsetImage <- function(extracted,peaks,percentage.deiso=3, thres.int=10000,
  ...
  temp.image <- cbind(as.numeric(finalmz),matrix(0, nrow=length(finalmz),
                                                 ncol=length(subset)))
  marker <- 0
...
contructImage <- function(extracted, deisotoped, peaks, imzMLparse,
  ...
  image <- cbind(as.numeric(d.finalmz),
    ...
    for(i in 1:length(standards))
      boxplot(as.vector(t(imagedata.in[standards[i],(offset+1):length(imagedata.in)])),
              main = paste("distribution of standard",
                           imagedata.in[standards[i],1],".",sep=" "))
  }
...
norm.TIC <- function(imagedata.in, offset){
  ...
  image.norm <- cbind(imagedata.in[,1:offset],
...
zeroperrow <- function(steps, matrix, plot.f=FALSE){
  ...
  if (plot.f) {
    plot(steps, results[1:(length(results))], ylab="Number of ions",
         xlab="% missing values",
...
imagePca <-function(imagedata.in, offset, PCnum, scale,x.cood,y.cood,nlevels,
  ...
  for(i in 1:PCnum){
    imageSlice(row=i, imagedata.in=t(pca$scores), scale, x.cood, y.cood,
    ...
    y   <- cut(pca$loadings[,i],
               breaks=c(-Inf,quantile(pca$loadings[,i],p=c(perc/100)),
    ...
    plot(x =as.numeric(as.vector(imagedata.in[,1])), y =pca$loadings[,i],
         type="n", main=paste("PC",i, " loadings",sep=""), xlab = "m/z",
         ylab="p[4]")
    lines(x =as.numeric(as.vector(imagedata.in[,1])), y =pca$loadings[,i],
          type="h")
    textxy(X =as.numeric(as.vector(imagedata.in[,1])), Y =pca$loadings[,i],
                    ...
                    offset=4)
{
  ...
  if (process==T){
    ...
    temp.image <- subsetImage(extracted, peaks, percentage.deiso,
                              thres.int, thres.low, thres.high, files,
                              spectra_dir,imzMLparse)
    temp.image.filtered <- filter(imagedata.in=temp.image, steps,
    ...
    annotated <- annotate(ionisation_mode, deisotoped, adducts,ppm.annotate,
                          dbase)

    ## make full image and add lipid ids
    final.image <- contructImage(extracted, deisotoped, peaks, imzMLparse,
    ...
    ## normalise image
    image.norm <- normalise(imagedata.in=image.ann, norm.type,
                            standards,offset=4)
  ...
  if (pca==T) {
    image.scale <- centreScale(imagedata.in = image.process, scale.type,
                               transform, offset = 4)

    imagedata.in = image.scale

    imagePca(imagedata.in=image.scale, offset=4,  PCnum, scale, x.cood,
  ...
  if(slice==T){
    imageSlice(row, imagedata.in=image.process, scale, x.cood, y.cood,
  ...
  if(cluster.k==T){
    cluster(cluster.type, imagedata.in=image.process, offset, width=x.cood,
